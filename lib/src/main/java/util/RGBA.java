...
public class RGBA {
	...
	public RGBA(double red, double green, double blue,double alpha) {
		try {
			if(red >255.0 || blue >255.0 || green > 255.0 || alpha > 1) throw new InvalidRGBAValueException("RGB value cannot exceed 255.0 and alpha value cannot exceed 1");
			if(red <0 || blue < 0 || green < 0 || alpha < 0) throw new InvalidRGBAValueException("RGBA value cannot be less than 0");
			this.setRed(red);
			this.setGreen(green);
			this.setBlue(blue);
			this.setAlpha(alpha);
